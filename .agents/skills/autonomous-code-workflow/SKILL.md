...
4. `uv run pytest -v --no-header <対象テスト>` (コミット直前は `uv run pytest -v --no-header --cov=lumi_companion`)
...
2. 要件適合性
3. 設計整合性
4. テスト妥当性・網羅性 (エッジケースカバー評価)
5. 簡潔性 (KISS)
6. 副作用
7. パフォーマンス
- **アトミック開発時**: `uv run pytest -v --no-header <対象テスト>` の関数別合否ログ。
- **最終完了・コミット前**: `uv run pytest -v --no-header --cov=lumi_companion` の全テスト ＋ カバレッジ表。
